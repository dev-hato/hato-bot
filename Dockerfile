...
RUN apt-get update && \
    ...
    if [ "${ENV}" = 'dev' ]; then \
      pipenv install --system --skip-lock --dev || for d in /tmp/pipenv-*-requirements/; do for f in ${d}/pipenv-*-reqs.txt;do cat $f done; done; \
    else \
      pipenv install --system --skip-lock || for d in /tmp/pipenv-*-requirements/; do for f in ${d}/pipenv-*-reqs.txt;do cat $f done; done; \
    fi

...
RUN apt-get update && \
    ...
    if [ "${ENV}" = 'dev' ]; then \
      pipenv install --system --skip-lock --dev || cat $(find /tmp/pipenv-*-requirements/pipenv-*-reqs.txt); \
    else \
      pipenv install --system --skip-lock ||  cat $(find /tmp/pipenv-*-requirements/pipenv-*-reqs.txt); \
    fi

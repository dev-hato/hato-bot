...
def get_jma_amedas(lat: float, lon: float):
    ...
    for code, place in place_res.json().items():
        ...
        place['distance'] = math.sqrt((decimal_lat - lat) ** 2 + (decimal_lon - lon) ** 2)
    ...
    amedas_url = latest_datetime.strftime(f'https://www.jma.go.jp/bosai/amedas/data/map/%Y%m%d%H%M%S.json')
